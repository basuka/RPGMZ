 ...
 *
 * このプラグインは、二人以上で使用する連携スキルコマンドを提供します。
 *
 * ■事前設定
 *
 *  1.データベースにあるタイプから、連携スキルに使用するスキルタイプを設定します。
 *  ※ここで設定したスキルタイプ名が連携スキルの項目名になります。
 *
 *  2.データベースにあるスキルから、連携スキルにするスキルを設定します。
 ...
 *   この項目が未入力、又は「0」が設定されている場合は、【スキル】で設定したスキル(データベース)の消費TPを設定されます。
 *
 *
 * ■計算式の設定方法
 * 
  ...
  Alignment_Skill.prototype.makeMenuItemList = function (windowSkillList) {
    ...
    for (stypeId in this._data) {
      ...
      for (alignmentSkill of alignmentSkills) {
        if (!this.isDummyData(alignmentSkill) && windowSkillList.includes(alignmentSkill)) {
  ...
  Window_SkillType.prototype.update = function() {
    ...
    if (this._skillTypeHelp) {
      if ($alignmentSkill.isAlignmentSkillType(this.currentExt())) {
        this._helpWindow.setText("決定キーで連携スキルに必要なメンバー情報と切り替えることが出来ます");
      } else {
