 ...
 *
 * また、当プラグインは「RPGツクールMV」に付属されている準公式プラグインを
 * MZ用に改変したものであり、その他の利用規約に関してはツクールシリーズの
 * 利用規約に従って下さい。
 ...
 * @decimals 1
 * @default 0.5
 * @desc 期待値を0〜1の間で設定します
 *       1に近づくほど当たりやすくなりますが、確実に当たるわけではありません
 *
 * @arg scale
 * @text 倍率
 * @type select
...
(() => {
    ...
    PluginManager.registerCommand(pluginName, "open", slotParams => {
        ...
        expectation = slotParams.expectation;
    ...
    const variableId = params.variableID || 11;
    const helpMessage = params.helpText || "カーソルキーの上でベット、カーソルキーの下でスタート";
    const winMessage = params.wonText || "おめでとうございます！Win Coin枚獲得です！";
    const lostMessage = params.lostText || "残念でした。";
    const replayMessage = params.replayText || "もう一度やりますか？";
    const coinFullMessage = params.coinFullText || "コイン枚数が制限に達しました。";
    const betText = params.betText || "ベット";
    const spinText = params.spinText || "スピン";
    const yesText = params.yesText || "はい";
    const noText = params.noText || "いいえ";
    let expectation = 0;
    let scale = 0;
    ...
    Scene_SlotMachine.prototype.createInstruction = function() {
        ...
        this._instructionWindow = new Window_SlotInstruction(rect);
        this._instructionWindow.setOdds(odds);
        this.addWindow(this._instructionWindow);
    };

    Scene_SlotMachine.prototype.instructionWindowRect = function() {
        const wx = 0;
        const wy = 0;
    ...
    Scene_SlotMachine.prototype.createSlotMachine = function() {
        const rect = this.slotMachineWindowRect();
        this._slotMachineWindow = new Window_SlotMachine(rect);
        this._slotMachineWindow.coin = this._coin;
        this._slotMachineWindow.bet = this._bet;
        this.addWindow(this._slotMachineWindow);
    };

    Scene_SlotMachine.prototype.slotMachineWindowRect = function() {
        const wx = 0;
        const wy = this._instructionWindow.height;
        const ww = Graphics.boxWidth;
        const wh = 200;
        return new Rectangle(wx, wy, ww, wh);
    };
    ...
    Scene_SlotMachine.prototype.createSlotCommand = function() {
        const rect = this.slotCommandWindowRect();
        this._slotCommandWindow = new Window_SlotCommand(rect);
    ...
    Scene_SlotMachine.prototype.slotCommandWindowRect = function() {
        const wx = 0;
        const wy = 0;
        const ww = Graphics.boxWidth;
    ...
    Scene_SlotMachine.prototype.createReplayCommand = function() {
        const rect = this.replayCommandWindowRect();
        this._replayCommandWindow = new Window_ReplayCommand(rect);
    ...
    Scene_SlotMachine.prototype.replayCommandWindowRect = function() {
        const wx = 0;
        const wy = 0;
        const ww = 240;
    ...
    Window_SlotInstruction.prototype.initialize = function(rect) {
    ...
    Window_SlotInstruction.prototype.drawText = function(text, x, y, maxWidth, align) {
    ...
    Window_SlotMachine.prototype.initialize = function(rect) {
        Window_Base.prototype.initialize.call(this, rect);
    ...
    Window_SlotCommand.prototype.initialize = function(rect) {
        this._betAllow = true;
    ...
    Window_ReplayCommand.prototype.initialize = function(rect) {
        Window_HorzCommand.prototype.initialize.call(this, rect);
        this.openness = 0;
    };
